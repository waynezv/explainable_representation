...
from nglcm_cepstra_source.nglcm import AlexNet, FilterNet
from nglcm_cepstra_source.trainer import trainer
from utils.datasets import prepare_data, SpokenDigitsSpecCeps
...
# Data func
yield_train_test = prepare_data(args['spectrogram']['list'],
...
for i in range(args['num_runs']):  # num_runs = C(num_folds, 3)
    ...
    train_data = SpokenDigitsSpecCeps(train_list,
                                      root_spec=args['spectrogram']['path'],
                                      root_ceps=args['cepstra']['path']
                                      )
    test_data = SpokenDigitsSpecCeps(test_list,
                                     root_spec=args['spectrogram']['path'],
                                     root_ceps=args['cepstra']['path']
                                     )
    ...
    filnet = FilterNet().to(args['device'])
